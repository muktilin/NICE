...
class RelationSampling(object):
    ...
    def prepare_just_bg_test_pairs(self, device, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            inters = boxlist_iou(proposal, proposal)
            rel_overs = inters[tgt_bg_idxs[:, 0], tgt_bg_idxs[:, 1]]
            inc = torch.where(rel_overs > 0.0)[0]

            if inc.shape[0] > 0:
                tgt_bg_idxs = tgt_bg_idxs[inc]
            ...
            if tgt_bg_idxs.shape[0] > 0 and inc.shape[0] > 0:
                rel_idx_pairs.append(tgt_bg_idxs)
                rel_labels.append(torch.zeros(tgt_bg_idxs.shape[0], device=device).long())
        return proposals, rel_labels, rel_idx_pairs
    ...
    def gtbox_non_bg_relsample(self, proposals, targets):
        assert self.use_gt_box
        num_pos_per_img = int(self.batch_size_per_image * self.positive_fraction)
        rel_idx_pairs = []
        rel_labels = []
        ...
        return filter_proposals, rel_labels, rel_idx_pairs, rel_sym_binarys
